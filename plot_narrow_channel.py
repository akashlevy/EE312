...
Ws, Ls, Vts = [], [], []
for f in glob('data/W1_D3_W*_L*_NMOS_VgId_loVd.txt'):
...
# Plot data
plt.title('Threshold voltage ($V_T$) at different channel widths')
plt.xlabel('Channel Width $W$ (um)', **axis_font)
plt.ylabel('$V_T$ (V)', **axis_font)
#plt.ylim(0,2)
plt.semilogx(Ws, Vts, '.', markersize=10)
plt.savefig('figures/narrow_channel.eps', format='eps', dpi=1000)
plt.show()